...
export default function RoleEdit() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Fetch permissions and all roles
        ...
        const role = rolesRes.data.find((r: any) => r.id === id)
        if (role) {
          setFormData({
            name: role.name,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const token = localStorage.getItem("token")
      await axios.put(`${API_URL}/roles/${id}`, formData, {
      ...
      <div className="px-4 md:px-6 lg:px-8 pt-4 pb-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-0.5">Ubah data peran dan atur ulang hak aksesnya.</p>
          </div>
        </div>
      </div>
